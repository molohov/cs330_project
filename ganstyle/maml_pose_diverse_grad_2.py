  ...
  def construct_model(self,
    ...
    with tf.variable_scope('model', reuse=None) as training_scope:
      ...
      def task_metalearn(inp, reuse=True):
        ...
        grad_updates.append([fast_weights[key] - weights[key] for key in weights.keys()])
        ...
        task_output = [
            task_outputa, task_outputbs, task_lossa, task_lossesb, task_msesb, grad_updates
        ]
      ...
      out_dtype = [
          tf.float32, [tf.float32], tf.float32, [tf.float32], [tf.float32], [[tf.float32] * 18]
      ]
      ...
      outputas, outputbs, lossesa, lossesb, msesb, grad_updates = result
      ...
      result_f = tf.map_fn(task_metalearn, elems=(self.input_fake, self.inputb, \
                                                ...
                                                parallel_iterations=FLAGS.meta_batch_size)
      outputas_fake, outputbs_fake, _, _, _, grad_updates_f = result_f
    ...
    if 'train' in prefix:
      ...
      if FLAGS.metatrain_iterations > 0:
        ...
        reg = 0
        for batch in range(len(grad_updates)):
          dist = 0
          #print(grad_updates[batch])
          for grad_update in range(len(grad_updates[batch])):
            # compute distance of grad update 
            diff = tf.square(grad_updates[batch][grad_update] - grad_updates_f[batch][grad_update])
